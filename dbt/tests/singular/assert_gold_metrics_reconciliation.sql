/*
  ...
  Validates that Gold total_spend aggregations match the Silver work order totals
  (closed + partial orders only) within a 1% tolerance per supplier per month.

  Returns rows where the variance exceeds tolerance — a passing test returns 0 rows.
*/
WITH silver_aggregated AS (
    SELECT
        FORMAT(order_date, 'yyyy-MM')   AS metric_month,
        p.supplier_id,
...
WHERE
    -- Flag rows where spend variance exceeds 1%
